...
class MusicBot(discord.Client):
    ...
    async def _auto_summon(self):
        ...
        if owner:
            self.safe_print("Tìm thấy chủ ở \"%s\", Tự động vào..." % owner.voice_channel.name)
            # TODO: Effort
    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if self.config.now_playing_mentions:
                newmsg = '%s - Bài **%s** đang được chơi ở channel %s!' % (
                    entry.meta['author'].mention, entry.title, player.voice_client.channel.name)
            else:
                newmsg = 'Đang bật bài ở kênh %s: **%s**' % (
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if 'entries' in info:
            ...
            reply_text = "Xếp bài **%s** vào playlist. Chơi sau **%s** bài"
            ...
            reply_text = "Cho **%s** vào playlist. Chơi sau %s"
            btext = entry.title
    ...
    async def _cmd_play_playlist_async(self, player, channel, author, permissions, playlist_url, extractor_type):
        ...
        if not info:
            raise exceptions.CommandError("Playlist này không chơi đụo.")
    ...
    async def cmd_restart(self, channel):
        await self.safe_send_message(channel, "địt mẹ rồi")
        await self.disconnect_all_voice_clients()
        raise exceptions.RestartSignal

    async def cmd_shutdown(self, channel):
        await self.safe_send_message(channel, "bố cút đây")
        await self.disconnect_all_voice_clients()
